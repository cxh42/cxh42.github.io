...
colors:
  ...
  school-maroon: "#500000"
  school-purple: "#4b2e83"
  school-blue: "#0b4ea2"
typography:
  display:
    fontFamily: "Geologica Variable, Geologica, system-ui, sans-serif"
    fontSize: "clamp(3.5rem, (min(100vw, 1360px + rail + 2 * gutter) - 2 * gutter - rail) / 7.4, 13rem)"
    fontWeight: 300
    lineHeight: 0.9
...
components:
  ...
  cjk-name:
    ...
    typography: "{typography.cjk-name}"
  hero-equation:
    textColor: "{colors.graphite}"
  ...
  edu-index:
    textColor: "{colors.on-screen-2}"
    typography: "{typography.small}"
    height: "{spacing.edu-row}"
  edu-index-current:
    textColor: "{colors.on-screen}"
  edu-index-marker:
    backgroundColor: "{colors.on-screen}"
    width: "3px"
    height: "16px"
  colophon-mark:
    textColor: "{colors.ink}"
...
The page behaves like one diffusion sampling run. Every state that changes arrives as a short noise-to-signal trajectory: per-pixel Gaussian grain (PCG hash into Box-Muller, WebGL2) resolving coarse shape first and crisp edges last, on the cosine ᾱ schedule, while a KaTeX readout of the live timestep counts down to t = 0. The name and an authored particle profile are sampled out of noise together in the first viewport; the profile diffuses back into noise as the reader scrolls. ViTeX scene switches, the campus hand-overs, the visitor globe and the Light/Dark switch use the same grammar. When a trajectory ends, its grain is gone: the settled page is clean ground.
...
Confirmed rejections: bordered, rounded or shadowed cards, pills, decorative shadows, small eyebrows or kickers above headings, glow, gradients on the accent, neon or cyber styling, standing grain or texture at rest, filters or blur on text, and any hover motion beyond colour and underline changes.
...
- One shared 24 fps clock; every sampler, the Chinese name's typing and every stepped CSS transition land on its frames.
- Hairline instruments (1px rules, 9px ticks, 1px underlines, 3×16px ink markers) and one flat plate-2 tint panel instead of boxes.
...
A near-monochrome instrument palette, off-white plate and cool ink, with one cobalt signal that exists only while something is live, and three school colours that live only inside the screen band.

### Primary
- **Sampling Cobalt** (accent; dark-accent on void; on-screen-accent in the dark band): the live timestep and the visitor. It colours a t or ᾱ_t readout only while t > 0, and visitor marks on the globe and the visit figure. At t = 0 readouts return to graphite. Functional affordances borrow it too: the focus ring, the text caret-color and the selection tint (select / dark-select). No glow, no gradient, no fills, no position markers.

### Tertiary
- **School Filters** (school-maroon for Texas A&M, school-purple for UW, school-blue for Henan): colour filters over the campus photographs in the screen band, never type, rules or UI. Live path: in the shader, luminance × (tint × 2.4 + 0.1), mixed at 0.6 over the toned photo. Static path: a tint layer with mix-blend-mode: color at 0.7 opacity. During a hand-over the tint crossfades under the noise.

### Neutral
- **Plate** (plate): the light ground. Flat at rest; samplers draw over it only while they run.
- **Tint Plate** (plate-2): the one tonal step and the only container fill: the publication and project panels. Media inside a panel sits on it directly.
- **Ink** (ink): names, headings, primary text, pressed and current states, the rail marker, the current research status, the portrait's particles, the colophon x₀.
- **Soft Ink** (ink-2): body prose, lede, authors, subtitles, venue track, area notes, and 陈星昊 with its caret.
- **Graphite** (graphite): instrumentation at rest: meta lines, dates, counts, rail labels, idle readouts, unpressed toggles, research statuses, the hero equation.
- **Hairline** (rule) and **Strong Hairline** (rule-strong): dividers and underline colour at rest (rule); rail ticks, theme separator, scrollbar thumb, resting link underline and the rule under each venue header (rule-strong).
...
- **Screen** (screen; screen-dark in dark theme), **On-Screen** (on-screen, on-screen-2, on-screen-rule, on-screen-rule-strong): the always-dark Education band. It stays dark in both themes; any rail label, tick or marker that sits over the band repaints in on-screen values, element by element. The school index marker is on-screen ink.

### Named Rules
**The Live-Only Accent Rule.** Cobalt means "t > 0 right now" or "a visitor was here". Position ("you are here" on the rail or the school index) and anything at rest, including the colophon x₀, is ink or graphite.
...
**The Band-Only School Colour Rule.** The school colours filter photographs inside the screen band and appear nowhere else.
...
**Character:** A clean, slightly technical grotesque kept thin and tightly tracked at scale, set against textbook Computer Modern so the math reads as math. The brush-written Chinese name is the page's one hand-made voice, small and secondary to the grotesque name.

### Hierarchy
- **Display** (300, container-width / 7.4 clamped 3.5–13rem, line-height 0.9, -0.04em): the hero name only. On phones (≤760px) each word takes its own line at (100vw − 2 gutters) / 3.75. Never wraps within a word line.
- **CJK Name** (CJK hand, 0.19em of the Display name; 0.26em at ≤760px; line-height 1, 0.08em, ink-2): 陈星昊 on the surname's baseline, 0.2em after it, followed by its caret. No script label.
...
A centred container of 1360px plus a 150px rail allowance (rail-space, ≥1100px only), inset by a fluid gutter (clamp 20–72px). Sections breathe with clamp(88px, 13vh, 168px) above and clamp(56px, 8vh, 104px) below. Content sits on a 12-column grid (column gap clamp 16–32px, row gap 32px): section title in columns 1–3, body in 4–12 at ≥1280px; below that the title stacks above the body. Secondary splits inside panels are two-column (project name and meta left at 1fr, account right at 1.25fr; publication text 1.05fr / 1fr) and collapse at 900px.

Section order: Hero, Research, Publications, Projects, Education, News, Visitors, Contact.

The hero is a full-viewport field (min 100svh, 560px) with the name bottom-left and a hairline-topped foot: lede left, the faint equation right (hidden at ≤760px). The portrait is centred at 60% of the width on desktop (height min(94% of viewport, 60% of width)), and at 43% on phones in the upper half. Panels stack with clamp(16px, 2vw, 24px) between them and pad clamp(16px, 2.6vw, 36px). Project images run full-width across the top of their panel as a pair of 16:9 frames with an 8px gap, each captioned in Label below. Lists are ledgers: a top rule, then 1px bottom rules per row (22px padding for news, 16px for research areas, 9px for country counts). Research rows are three cells (name, status, optional note); at ≤760px the note drops to its own line. The Education band is full-bleed; with WebGL it becomes a sticky 100svh screen inside a 330svh scroll track, with the heading, a vertical school index (34px rows, 30px on phones) and the degree text stacked on its left.

Breakpoints observed: 420, 640, 760, 900, 1100 (rail appears, menu disappears), 1280 (margin headings).

## Elevation & Depth

Flat. Depth comes from sampling itself, the one tint step and the one dark screen band, not from lifting surfaces. Panels are tonal, never raised. The portrait canvas sits fixed behind the hero content and is clipped at the hero rule with a 56px soft edge, so nothing it draws reaches the foot or the sections below. Particles are cleared entirely behind 陈星昊 (12px margin) and thinned to 0.4 behind the English name, so neither competes with dots. The top bar gains a plate fill and a 1px hairline (`box-shadow: 0 1px 0 var(--rule)`) once scrolled; that is a rule drawn with box-shadow, not elevation.
...
**The Clean Ground Rule.** At rest the ground is flat plate or void. Grain and particles exist only while a trajectory runs; the hero canvas clears and stops once the name settles, every campus settles at t = 0, and the portrait never draws below the hero rule or behind 陈星昊.

## Shapes

Square everywhere: no radius on media, panels, rows, buttons or the menu. The only curve is the 2px radius of the focus outline (1.5px accent, 3px offset). Form is made of lines and one tint: 1px rules, rail ticks 9px at rest and 18px when current, 1px underlines under pressed toggles, the 1px vertical rule of the school index, the 3×16px ink markers on the rail and the school index, the vertical CJK caret (0.07em × 0.95em), and the square plate-2 panel. Project images are square-cornered 16:9 crops. Portrait particles are soft round points (2.0 CSS px × dpr, ±25%). Icons are inline SVG line drawings at 1.2–1.5px stroke (arrows, menu bars).
...
- **Schedule rail (≥1100px):** fixed right, vertically centred, 120px wide, 18px between rows, listing every section including Projects. Every section is always labelled (Label size, graphite). The current section is ink at weight 500 and its tick grows from 9px to 18px in ink, done with transform: scaleX (0.5 at rest) from the right edge over 200ms. The 3×16px ink marker glides on the 1px line between ticks with section progress. Each label, tick and the marker switch to on-screen values individually whenever they sit over the Education band.

### Chinese Name (signature)
陈星昊 in ink-2 follows "Chen" on its baseline, trailed by a vertical caret in currentColor. On the 24 fps clock, once the name has settled and all six faces are loaded: hold 84 frames (3.5s) with the caret blinking at 1Hz (12 frames on, 12 off), erase one character every 3 frames, a 14-frame gap in which the hand changes, then type one character every 6 frames. It appears with the hero foot and pauses when off-screen. Reduced motion shows the first hand, still, with no caret. It never sits over the English name.

### Hero Equation
The forward-diffusion equation and its t / ᾱ_t readout, right-aligned in the hero foot, graphite at 0.6 opacity: ornament, not an instrument. Hidden at ≤760px.

### Live Readouts (signature)
A KaTeX label (t=, ᾱ_t=) followed by a numeral in Notation type. Graphite at rest; accent (on-screen-accent in the band) while t > 0, with the colour change stepped (200ms, 5 steps). t is shown ×1000 as an integer; ᾱ_t to three decimals. Every sampler exposes one except the portrait, which shares the hero's.
...
- **Particle portrait:** an authored left-facing profile bust, not derived from any photograph: the outline plus five offset rings, each smoothed more than the last (about 4.4k points). Ink points at alpha 0.66 on light and 0.78 on dark, sampled over 44 steps alongside the name. On scroll (s = scrollY / 0.9 viewport heights) it is re-noised to t = 0.9·s^1.2, rises by 0.14 of its height and fades out between s = 0.12 and 0.55 (smoothstep). It stops drawing once past the hero. Reduced motion draws it once at rest.
...
- **Campus band:** the band arrives at t = 0.72 and samples the first campus clean over 22 frames. Crossing a scroll threshold (equal thirds of the pinned range) starts a timed 28-frame hand-over: forward to the peak over the first 40%, the next campus takes over under the noise, then back to t = 0; the school tint and exposure gain crossfade with it. Noise is scaled grain over a mip-blurred photo, toned dark, so it never passes through white. Every school settles at t = 0. Degree text switches by opacity (375ms, 9 steps).
- **Visitor globe:** points diffuse onto the sphere over 36 frames; the globe yaws slowly on the clock and can be dragged.

### Screen Band
Education, always dark (screen / on-screen). Static path: each school is its own full-bleed plate with its photograph toned (grayscale 0.35, brightness 0.52 × the school's exposure, contrast 1.05), its school colour laid over with mix-blend-mode: color, and a bottom scrim. Live path: one sticky screen, the photograph sampled in WebGL (toned, school-filtered, exposure-scaled, scrimmed under the degree, the heading and index, and the rail). Per-school exposure is 1.45 for Texas A&M and 1 otherwise.

### School Index
A vertical list beside a 1px rule: short school name (Small) and year (Label, tabular) per row, on-screen-2 at rest, on-screen with the name at 500 when current. A 3×16px on-screen marker slides between rows (420ms). Each row is a button that scrolls to that school's stretch of the band. The band's small t readout sits beneath it.

### Ledger Rows
News, research areas and country counts: a top rule, 1px bottom rule per row. News and counts carry a graphite date or count column in tabular numerals with ink text. Research areas are three cells: name (Lead, 400, ink), status (Label, graphite; the current area's status in ink at 500), optional note (Small, ink-2).

### Colophon
A hairline-topped footer: the x₀ Colophon Mark in ink at left, Label-size meta and credits at right.

### Motion Grammar
- **Clock:** 24 fps (one frame = 41.667ms). Samplers and the Chinese name only act on shared frame boundaries.
- **Stepped CSS:** state transitions that belong to the world use `steps(n)` sized to whole frames (200ms/5, 375ms/9, 625ms/15). Chrome transitions (link, toggle colour, rail label weight and tick, school marker, bar fill, wordmark) use 160–420ms on cubic-bezier(0.16, 1, 0.3, 1).
...
- **Do** clear and stop a sampler when its trajectory ends; the ground at rest is flat plate or void, and every campus rests at t = 0.
...
- **Don't** crossfade or slide where the world samples; no crossfades for theme or scene changes, and campus hand-overs cross under the noise, never through white.
- **Don't** leave grain, particles or texture standing on the page at rest, or let the portrait draw below the hero rule or behind 陈星昊.
- **Don't** use the school colours outside the screen band.
