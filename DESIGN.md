...
### Primary
- **Sampling Cobalt** (accent; dark-accent on void; on-screen-accent in the dark band): the live timestep and the visitor. It colours a t or ᾱ_t readout only while t > 0, and visitor marks on the globe and the visit figure. At t = 0 readouts return to graphite and then fade out. Functional affordances borrow it too: the focus ring, the text caret-color and the selection tint (select / dark-select). No glow, no gradient, no fills, no position markers.
...
The hero is a full-viewport field (min 100svh, 560px) on its own ground, `--hero-ground`: a soft radial light centred behind the portrait (white falling to #e9e9e5 on light, #17171c falling to #060607 on dark), closed by a 1px rule, so the first viewport reads as a surface of its own above the flat plate. It holds the name bottom-left and a hairline-topped foot: lede left, the faint equation right (hidden at ≤760px). The portrait is centred at 60% of the width on desktop (height min(94% of viewport, 60% of width)), and at 43% on phones in the upper half. Panels stack with clamp(16px, 2vw, 24px) between them and pad clamp(16px, 2.6vw, 36px). Project images run full-width across the top of their panel as a pair of 16:9 frames with an 8px gap, each captioned in Label below. Lists are ledgers: a top rule, then 1px bottom rules per row (22px padding for news, 16px for research areas, 9px for country counts, each country row carrying its visited regions beneath in Label graphite). Research interests are a ledger of names, with a single ink "Current work" marker on the current one. The Education band is full-bleed; with WebGL it becomes a sticky 100svh screen inside a 100svh + 2×70svh scroll track, with the heading, a vertical school index (full school name, degree abbreviation, year range; 34px rows, 30px on phones) and the degree text stacked on its left.
...
A KaTeX label (t=, ᾱ_t=) followed by a numeral in Notation type. Accent (on-screen-accent in the band) while t > 0, with the colour change stepped (200ms, 5 steps). t is shown ×1000 as an integer; ᾱ_t to three decimals. Every sampler exposes one except the portrait, which shares the hero's. A readout is present only while its number moves: each new value brings it up (180ms), and 650ms after the last change it fades out (1.4s ease-out), so a settled page shows no t at all (`tReadout` in gl.ts, `.t-read` / `.t-on`).
